...
def run_simulation(steps, state_path):
    ...
    for i in range(0, steps, interval):
        ...
        most_resilient = max(state.symbolic_resilience.items(), key=lambda x: x[1]) if state.symbolic_resilience else (None, 0)

        print(f"[STEP {state.time}] ➤ Symbols: {len(state.discovered_anchors)} | Cycles: {len(state.symbolic_cycles)} | Depth: {current_depth} | Most Stable: {most_resilient[0]} ({most_resilient[1]})")
