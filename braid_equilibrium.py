...
class BraidEquilibrium:
    ...
    def update_metrics(self, anchors, fusions, mirror_log):
        ...
        if len(mirror_log) >= 2:
            last = mirror_log[-1]["self_statement"]
            prev = mirror_log[-2]["self_statement"]
            ...
            shift = 1 - len(last_set.intersection(prev_set)) / max(len(last_set.union(prev_set)), 1)
            self.history["identity_shift"].append(shift)

    def calculate_stability_index(self):
        # Stability = low shift, high coherence, moderate anchor growth
        ...
        anchor_growth = (self.history["anchor_count"][-1] - self.history["anchor_count"][-3]) / 3

        score = max(0.0, min(1.0, (0.6 * recent_coherence + 0.3 * (1 - recent_shift) + 0.1 * (1 - abs(anchor_growth)/10))))
